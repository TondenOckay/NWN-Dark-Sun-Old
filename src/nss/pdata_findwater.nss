...
void main()
{
    ...
    int nLastFindWaterTime = h2_GetModLocalInt(h2_GetPlayerPersistentString(oPC, H2_UNIQUE_PC_ID) + H2_LAST_PC_FINDWATER_TIME);
    ...
    // If not in combat, and hasn't checked for water since the last full rest, go ahead and try to find some now
        ...
        int iAreaType = GetLocalInt(GetArea(oPC), H2_HTF_AREATYPE);
        ...
        h2_SaveLastFindWaterTime(oPC);

        // Apply speed decrease for 120 seconds
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eEffect, oPC, 10.0);

        //Execute SearchForWaterFunction with 10 second
        DelayCommand(10.0, h2_SearchForWater(oPC, iAreaType));
    }
}
