...
void ParseEmote(string sEmote, object oPC)
{
    ...
    //_AID_ -------- MAGUS -----------------------------------------------------
    int bAIDSuccess;
    // _AID_ uncomment the line below if you are using AID
    //bAIDSuccess = AIDParseEmote(oPC, sLCEmote);
  ...
  if(DMFI_EMOTE_SAVES)
  {
    ...
    else if ((FindSubString(sLCEmote, " will save ") != -1))
    ...
    // has _AID_ already caused an animation? if TRUE, exit.
    if(bAIDSuccess){return;}
    ...
    else if (FindSubString(sLCEmote, " hooray ")!= -1)
    {
        PlayVoiceChat(VOICE_CHAT_CHEER, oPC);
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_VICTORY2, 1.0));
    }
    else if (FindSubString(sLCEmote, " celebrat")!= -1)
    {
        PlayVoiceChat(VOICE_CHAT_CHEER, oPC);
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_VICTORY3, 1.0));
    }
    else if (FindSubString(sLCEmote, " giggl")!= -1 && GetGender(oPC) == GENDER_FEMALE)
        AssignCommand(oPC, PlaySound("vs_fshaldrf_haha"));
    else if (FindSubString(sLCEmote, " flop")!= -1 ||
             FindSubString(sLCEmote, " prone")!= -1)
        ...
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_GET_MID, 1.0, 5.0));
             ...
             FindSubString(sLCEmote, " agree")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_LISTEN, 1.0, 4.0));
             ...
             FindSubString(sLCEmote, " search")!= -1
            )
             ...
             FindSubString(sLCEmote, " meditate")!= -1)
    {
        ...
        AssignCommand(oPC,ActionPlayAnimation(ANIMATION_LOOPING_MEDITATE, 1.0, 99999.0));
             ...
             FindSubString(sLCEmote, " woozy ")!= -1)
             ...
             FindSubString(sLCEmote, " exhausted ")!= -1)
    {
        ...
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_PAUSE2, 1.0, 99999.0));
             ...
             (FindSubString(sLCEmote, " floor")!= -1 ||
              FindSubString(sLCEmote, " ground")!= -1))
        ...
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_TALK_FORCEFUL, 1.0, 99999.0));
             ...
             FindSubString(sLCEmote, " giggl")!= -1
           )
    {
    ...
    else if (FindSubString(sLCEmote, " talks ")!= -1 ||
             FindSubString(sLCEmote, " chats ")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_TALK_NORMAL, 1.0, 99999.0));
    else if (FindSubString(sLCEmote, " shakes ") != -1
            && FindSubString(sLCEmote, " head ")>FindSubString(sLCEmote, " shakes ")
           )
    {
    ...
    else if (FindSubString(sLCEmote, " falls back")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_DEAD_BACK, 1.0, 99999.0));
    else if (FindSubString(sLCEmote, " spasm")!= -1 || FindSubString(sLCEmote, " shakes ") != -1)
    ...
    else if (FindSubString(sLCEmote, " dance")!= -1 || FindSubString(sLCEmote, " dancing ")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_CUSTOM19, 2.0, 99999.0));
    */
}
